...
export interface BloodsDataType {
  id: string
  nama: string
}
...
function useBloods(params: IParams = {}) {
  const { page, pageSize, options, ...restParams } = params
  const [pagination, setPagination] = useState({ page, pageSize })

  const queryKey = ['bloods', pagination, restParams]

  const query = useQuery({
    queryKey,
    queryFn() {
      return CallAPI.getMasterBlood({ ...restParams, ...pagination }).then(
        (res) => {
          return res.data
        },
      )
